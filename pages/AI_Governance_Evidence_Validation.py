...
def stratified_pilot_sample(
    ...
    groups = [
        index.to_numpy()
        for _, index in corpus.groupby(stratum_key, sort=True).groups.items()
    ]
    group_sizes = np.array([len(group) for group in groups], dtype=int)

    selected: list[int] = []
    if len(groups) <= target:
        # Give every stratum one unit, then allocate the remaining units
        # proportionally. The largest-remainder rule keeps the final sample
        # balanced and reproducible when equally sized strata are used.
        quotas = np.ones(len(groups), dtype=int)
        remaining = target - len(groups)
        capacities = group_sizes - quotas

        if remaining > 0 and capacities.sum() > 0:
            ideal_extras = remaining * capacities / capacities.sum()
            extras = np.floor(ideal_extras).astype(int)
            quotas += extras
            remainder = remaining - int(extras.sum())

            if remainder > 0:
                fractional = ideal_extras - extras
                tie_break = rng.random(len(groups))
                allocation_order = np.lexsort((tie_break, -fractional))
                for group_position in allocation_order:
                    if remainder == 0:
                        break
                    if quotas[group_position] < group_sizes[group_position]:
                        quotas[group_position] += 1
                        remainder -= 1

        for group_indices, quota in zip(groups, quotas):
            chosen = rng.choice(group_indices, size=int(quota), replace=False)
            selected.extend(int(index) for index in chosen)
    else:
        # When there are more strata than pilot places, select strata in
        # proportion to their size and take one unit from each selected stratum.
        probabilities = group_sizes / group_sizes.sum()
        ...
        for group_position in chosen_groups:
            selected.append(int(rng.choice(groups[int(group_position)])))
...
with corpus_tab:
    ...
    if corpus.empty:
        ...
        strata = st.multiselect(
            ...
            default=[
                column
                for column in ["Proposed_Governance_Domain", "Sector"]
                if column in available_strata
            ],
        )
        st.caption(
            "For this 288-unit GCC corpus, use a minimum of 32 units and "
            "stratify by Proposed_Governance_Domain and Sector. This provides "
            "two banking and two telecommunications units for every domain."
        )
