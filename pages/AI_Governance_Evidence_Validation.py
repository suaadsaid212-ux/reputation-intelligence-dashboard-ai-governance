...
def evidence_matrix(final_coding: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    counts["Total_Coded_Claims"] = counts[level_columns].sum(axis=1)
    for column in level_columns:
        counts[f"Percent_{column}"] = (
            counts[column] / counts["Total_Coded_Claims"] * 100
        ).round(1)
    ...
    coverage = coverage_index.merge(
        counts[["Organization", "Governance_Domain", "Total_Coded_Claims"]],
        ...
        coding_editor = st.data_editor(
            ...
            column_config={
                ...
                "Evidence_Level": st.column_config.SelectboxColumn(
                    ...
                    required=True,
                    help="1 Aspirational, 2 Procedural, 3 Implemented, 4 Verified",
                ),
