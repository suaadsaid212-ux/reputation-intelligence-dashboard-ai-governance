...
def install_global_translation():
    ...
    text_methods = [
        ...
        "success", "error", "markdown", "toast", "spinner", "expander", "popover",
    ]
    ...
    for name in text_methods:
        if hasattr(DeltaGenerator, name):
            ...
            def make_dg(func):
                ...
                return wrapper
            setattr(DeltaGenerator, name, make_dg(original))
        ...
        if hasattr(st, name):
            ...
            def make_st(func):
                ...
                return wrapper
            setattr(st, name, make_st(original))

    if hasattr(DeltaGenerator, "write"):
        original = DeltaGenerator.write
        def dg_write(self, *args, **kwargs):
            args = tuple(translate(x) if isinstance(x, str) else x for x in args)
            return original(self, *args, **kwargs)
        DeltaGenerator.write = dg_write
    ...
    if hasattr(DeltaGenerator, "metric"):
        ...
        def dg_metric(self, label, value, *args, **kwargs):
        ...
        DeltaGenerator.metric = dg_metric
    ...
    for name in widget_methods:
        if hasattr(DeltaGenerator, name):
            original = getattr(DeltaGenerator, name)
            def make_widget(func, method_name):
                def wrapper(self, *args, **kwargs):
                    args, kwargs = _translate_first(args, kwargs)
                    if method_name in {"selectbox", "multiselect", "radio"}:
                        kwargs["format_func"] = _translated_format_func(kwargs.get("format_func"))
                    return func(self, *args, **kwargs)
                return wrapper
            setattr(DeltaGenerator, name, make_widget(original, name))
        ...
        if hasattr(DeltaGenerator, name):
            ...
            def make_data(func):
                def wrapper(self, data=None, *args, **kwargs):
                    return func(self, translate_dataframe(data), *args, **kwargs)
                return wrapper
            setattr(DeltaGenerator, name, make_data(original))

    if hasattr(DeltaGenerator, "plotly_chart"):
        original = DeltaGenerator.plotly_chart
        def dg_plotly(self, fig, *args, **kwargs):
            return original(self, _translate_plotly(fig), *args, **kwargs)
        DeltaGenerator.plotly_chart = dg_plotly

    if hasattr(DeltaGenerator, "pyplot"):
        original = DeltaGenerator.pyplot
        def dg_pyplot(self, fig=None, *args, **kwargs):
            if fig is not None:
                fig = _translate_matplotlib(fig)
            return original(self, fig, *args, **kwargs)
        DeltaGenerator.pyplot = dg_pyplot

    if hasattr(st, "set_page_config"):
        original = st.set_page_config
        def config_wrapper(*args, **kwargs):
            if isinstance(kwargs.get("page_title"), str):
                kwargs["page_title"] = translate(kwargs["page_title"])
            return original(*args, **kwargs)
        st.set_page_config = config_wrapper
